"""Script tạo lại notebook Model RAF.ipynb - FIXED v2."""
import json, os, shutil

def cell(ct, src):
    c = {"cell_type": ct, "metadata": {}, "source": src if isinstance(src, list) else [src]}
...
cells.append(code(["model_name = 'RAF'"]))
cells.append(md(["Load pretrained model to fine-tune it? If yes, specify model name, if no set it to `None`."]))
cells.append(code(["pretrained_model_name = None"]))
cells.append(md(["# Data"]))
cells.append(md(["## Preprocessing Function\n",
...
cells.append(md(["## Confusion Matrix & Classification Report"]))
cells.append(code([
...
cells.append(md(["## Per-class Accuracy"]))
cells.append(code([
...
nb = {"cells": cells, "metadata": {"kernelspec": {"display_name": "Python 3", "language": "python", "name": "python3"},
    ...
    "pygments_lexer": "ipython3", "version": "3.11.5"}}, "nbformat": 4, "nbformat_minor": 5}

old = "Model RAF.ipynb"
bak = "Model RAF.backup.ipynb"
if os.path.exists(old): shutil.copy2(old, bak); print(f"Backup: {bak}")
with open(old, "w", encoding="utf-8") as f: json.dump(nb, f, indent=1, ensure_ascii=False)
print(f"Done! Created {old} with {len(cells)} cells")
